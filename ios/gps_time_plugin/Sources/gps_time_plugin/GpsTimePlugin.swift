...
public class GpsTimePlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "startListening":
            if gpsTimeHelper == nil {
                ...
                // Callback is set in onListen; here we only start listening
            }
    ...
    /// Connects the GpsTimeHelper callback to the active event sink.
...
extension GpsTimePlugin: FlutterStreamHandler {
    ...
    public func onListen(
        ...
        // Ensure the helper exists before wiring the callback
        if gpsTimeHelper == nil {
            gpsTimeHelper = GpsTimeHelper()
        }
        setupGpsCallback()
